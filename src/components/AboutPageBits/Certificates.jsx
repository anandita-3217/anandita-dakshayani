...
const CertificateDetailModal = ({ isOpen, onClose, cert }) => {
  ...
  // Check if certificate has a PDF
  const hasPdf = cert.certificatePdfUrl && cert.certificatePdfUrl.toLowerCase().endsWith('.pdf');

  return (
    <Modal isOpen={isOpen} onClose={onClose} size={hasPdf && showPdf ? "6xl" : "xl"} isCentered>
      <ModalOverlay bg="blackAlpha.700" backdropFilter="blur(10px)" />
      <ModalContent 
        maxW={hasPdf && showPdf ? "90vw" : "700px"}
          ...
          <HStack spacing={3} justify="space-between">
            ...
            {hasPdf && (
              <Button
                size="sm"
                colorScheme="blue"
                variant={showPdf ? "solid" : "outline"}
                onClick={() => setShowPdf(!showPdf)}
              >
                {showPdf ? "Show Details" : "Show PDF"}
              </Button>
            )}
          </HStack>
        </ModalHeader>
...
export default function Certificates () {
  ...
  const filteredCerts = certificatesData
    ...
    .sort((a, b) => {
      // Priority order: degree > course > certification
      const categoryOrder = { degree: 1, course: 2, certification: 3 };
      
      // First sort by category priority
      const categoryDiff = categoryOrder[a.category] - categoryOrder[b.category];
      if (categoryDiff !== 0) return categoryDiff;
      
      // Then sort chronologically within same category (newest first)
      // Parse years from dateEarned (handles formats like "2024", "Jan 2024", etc.)
      const getYear = (dateStr) => {
        const yearMatch = dateStr.match(/\d{4}/);
        return yearMatch ? parseInt(yearMatch[0]) : 0;
      };
      
      return getYear(b.dateEarned) - getYear(a.dateEarned);
    });
