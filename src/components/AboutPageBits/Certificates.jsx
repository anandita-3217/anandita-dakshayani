  ...
  return (
    ...
    >
      ...
      >
        <Box position="absolute" left={0} top={0} bottom={0} w="2px" bg={meta.accent}
          transform={hov ? 'scaleY(1)' : 'scaleY(0)'} transformOrigin="top" transition="transform 0.22s ease" />
        <Grid templateColumns="40px 1fr auto" gap={4} alignItems="center">
          {/* Row number — slightly larger */}
          <Text fontFamily="monospace" fontSize="12px" color={numColor} textAlign="right" userSelect="none">
            {String(index + 1).padStart(2, '0')}
          </Text>
          ...
          <Box minW={0}>
            <HStack spacing={3} mb={1.5}>
              {/* Category badge */}
              <Text fontFamily="monospace" fontSize="9px" letterSpacing="0.22em" color={meta.accent}>
                [{meta.label}]
              </Text>
              <Text fontFamily="monospace" fontSize="10px" color={metaCol}>{cert.dateEarned}</Text>
              <HStack spacing={1} color={metaCol}>
                <Clock size={10} />
                <Text fontFamily="monospace" fontSize="10px">{cert.duration}</Text>
              </HStack>
            </HStack>

            {/* Title — bumped from 12 → 14px */}
            <Text fontFamily="Orbitron" fontSize="14px" fontWeight="700" color={titleCol}
              noOfLines={1} letterSpacing="0.01em" mb={1}>{cert.title}</Text>

            {/* Institution — bumped from 11 → 13px */}
            <Text fontFamily="Sora" fontSize="13px" color={meta.accent} fontWeight="600" noOfLines={1} mb={2.5}>
              {cert.institution}
            </Text>

            <Flex flexWrap="wrap" gap={1.5}>
              {cert.skills.slice(0, 4).map((s, i) => (
                <Box key={i} px={2} py={1} borderRadius="4px" bg={tagBg} border="1px solid" borderColor={tagBorder}>
                  {/* Tag text — bumped from 8 → 10px */}
                  <Text fontFamily="monospace" fontSize="10px" color={metaCol} letterSpacing="0.03em">{s}</Text>
                </Box>
              ))}
              {cert.skills.length > 4 && (
                <Box px={2} py={1} borderRadius="4px" bg={tagBg} border="1px solid" borderColor={tagBorder}>
                  <Text fontFamily="monospace" fontSize="10px" color={metaCol}>+{cert.skills.length - 4}</Text>
                </Box>
              )}
            </Flex>
          </Box>
...
const TimelineEntry = ({ cert, index, onViewDetails }) => {
  ...
  const Card = () => (
    <Box
      bg={cardBg} border="1px solid" borderColor={cardBord} borderRadius="12px" p={5}
      cursor="pointer" transition="all 0.22s"
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      onClick={() => onViewDetails(cert)}
      boxShadow={hov ? `0 6px 24px ${meta.accent}14` : 'none'}
    >
      <HStack justify="space-between" align="start" mb={2.5}>
        <HStack spacing={2}>
          <Box color={meta.accent} opacity={0.8}>{cert.logo}</Box>
          <VStack align="start" spacing={0}>
            <HStack spacing={2}>
              {/* Category + date — bumped from 8/9 → 9/10px */}
              <Text fontFamily="monospace" fontSize="9px" letterSpacing="0.2em" color={meta.accent}>[{meta.label}]</Text>
              <Text fontFamily="monospace" fontSize="10px" color={metaCol}>{cert.dateEarned}</Text>
            </HStack>
            {/* Title — bumped from 11.5 → 13px */}
            <Text fontFamily="Orbitron" fontSize="13px" fontWeight="700" color={titleCol}
              letterSpacing="0.01em" noOfLines={2}>{cert.title}</Text>
          </VStack>
        </HStack>
        <Box color={meta.accent} opacity={hov ? 1 : 0.25} transition="opacity 0.2s" flexShrink={0} mt={0.5}>
      ...
      {/* Institution — bumped from 11 → 13px */}
      <Text fontFamily="Sora" fontSize="13px" color={meta.accent} fontWeight="600" mb={1.5}>{cert.institution}</Text>

      <HStack spacing={3} color={metaCol} mb={2.5}>
        <HStack spacing={1}>
          <Clock size={10}/>
          {/* Duration — bumped from 9 → 11px */}
          <Text fontFamily="monospace" fontSize="11px">{cert.duration}</Text>
        </HStack>
        {cert.grade && <Text fontFamily="monospace" fontSize="11px">{cert.grade}</Text>}
      </HStack>
      ...
      <Flex flexWrap="wrap" gap={1.5}>
        {cert.skills.slice(0, 3).map((s, i) => (
          <Box key={i} px={2} py={1} borderRadius="4px" bg={tagBg} border="1px solid" borderColor={tagBorder}>
            {/* Tag text — bumped from 8 → 10px */}
            <Text fontFamily="monospace" fontSize="10px" color={metaCol}>{s}</Text>
          </Box>
        ))}
        {cert.skills.length > 3 && (
          <Box px={2} py={1} borderRadius="4px" bg={tagBg} border="1px solid" borderColor={tagBorder}>
            <Text fontFamily="monospace" fontSize="10px" color={metaCol}>+{cert.skills.length - 3}</Text>
          </Box>
        )}
      </Flex>
    </Box>
...
export default function Certificates() {
  ...
  if (loading) return (
    <Box minH="1100px" bg="transparent" position="relative" overflow="hidden"
      display="flex" alignItems="center" justifyContent="center"
      px={{ base: 5, md: 12, lg: 20 }} py={{ base: 20, md: 0 }}
      fontFamily="'Sora', sans-serif" transition="background-color 0.3s ease">
      <VStack spacing={4}>
        <Spinner size="md" color="14b8a6" thickness="1px" />
        <Text fontFamily="monospace" fontSize="10px" letterSpacing="0.2em" color={subColor}>
          loading records...
