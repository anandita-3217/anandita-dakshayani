...
// ── Magnetic button ────────────────────────────────────────────────────────
function MagneticBtn({ children, href = "#", variant = "primary" }) {
  ...
  return (
    ...
    >
      <Box
        position="relative" px={7} py={3} borderRadius="full" overflow="hidden"
        border="1.5px solid"
        borderColor={isPrimary ? "transparent" : "rgba(255,255,255,0.12)"}
        cursor="pointer"
        style={{
          background: isPrimary ? "linear-gradient(135deg, #1e40af, #7c3aed, #ec4899)" : undefined,
      ...
      >
        {hovered && (
          <Box position="absolute" top={0} bottom={0} w="40px"
            bg="linear-gradient(90deg, transparent, rgba(255,255,255,0.25), transparent)"
            animation={`${shimmerLine} 0.55s ease forwards`}
            pointerEvents="none"
          />
        )}
        <Text fontFamily="'Sora', sans-serif" fontSize="13px" fontWeight={600}
          color="white" letterSpacing="0.06em" position="relative" zIndex={1}>
          {children}
        </Text>
      </Box>
    </motion.a>
