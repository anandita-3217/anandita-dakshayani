    ...
    <div style={{ fontFamily: "monospace", width: "100%" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            marginLeft: 12, fontFamily: "monospace", fontSize: 11, color: terminalLabel,
          ...
          <button
            onClick={startTyping}
            style={{
              marginLeft: "auto", fontFamily: "monospace", fontSize: 9, letterSpacing: "0.1em",
              color: clearBtnColor, background: "transparent", flexShrink: 0,
              border: `1px solid ${clearBtnBorder}`, borderRadius: 4,
              padding: "2px 8px", cursor: "pointer",
            }}
          >
        ...
        {/* Output — height scales with screen, min for very small */}
        <div
          ref={termRef}
          style={{
            padding: "16px 16px",
            ...
            overflowY: "auto",
            scrollbarWidth: "none",
            overflowX: "hidden",   // ← prevents horizontal blowout on mobile
          }}
        >
          {lines.map((line, i) => {
            const cs = line.cat ? catColors[line.cat] : null;
            ...
            if (line.type === "prompt") return (
              <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 0, marginBottom: 12 }}>
                <span style={{ color: "#14b8a6", fontSize: 12 }}>anandita</span>
                <span style={{ color: promptAt, fontSize: 12 }}>@</span>
                <span style={{ color: "#a855f7", fontSize: 12 }}>portfolio</span>
                <span style={{ color: promptSep, fontSize: 12 }}>:~$ </span>
                <span style={{ color: promptText, marginLeft: 4, fontSize: 12 }}>
                  {line.text.replace("anandita@portfolio:~$ ", "")}
                  {i === lines.length - 1 && (
                    <span style={{ opacity: cursor ? 1 : 0, color: "#14b8a6" }}>█</span>
                  )}
            ...
            if (line.type === "header") return (
              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                style={{
                  fontSize: 9, letterSpacing: "0.15em", color: cs.color,
            ...
            if (line.type === "skill") {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.25 }}
                  style={{ display: "flex", alignItems: "center", gap: 0, marginBottom: 6, fontSize: 12 }}>
                  {/* Arrow */}
                  <span style={{ color: cs.color, width: 14, flexShrink: 0 }}>▸ </span>
                  {/* Name — fixed narrow width on mobile, wider on desktop */}
                  <span style={{
                    color: skillName,
                    width: "clamp(80px, 25%, 130px)",
                    flexShrink: 0,
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                  }}>
                    {s.name}
                  </span>
                  {/* Version — hidden on very narrow viewports via CSS clamp trick */}
                  <span style={{
                    color: skillVersion,
                    width: "clamp(0px, 12%, 60px)",
                    flexShrink: 0,
                    fontSize: 10,
                  ...
                  <div style={{ flex: 1, height: 3, background: barTrack, borderRadius: 2, marginRight: 8, minWidth: 30 }}>
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${s.level}%` }}
                      transition={{ delay: 0.1, duration: 0.6, ease: "easeOut" }}
                  ...
                  {/* Percentage */}
                  <span style={{ color: cs.color, width: 32, textAlign: "right", fontSize: 10, flexShrink: 0 }}>
                    {s.level}%
                  </span>
                </motion.div>
              );
