...
// ── Icon registry — maps JSON string → lucide component ───────────────────
// Add new entries here whenever you add a new icon to reels.json
const ICON_MAP = {
  Paintbrush,
  CodeXml,
  Sparkles,
...
// ── ReelCarousel ──────────────────────────────────────────────────────────
// Props:
//   reels  — array of reel objects (from reels.json or a parent fetch)
//            tagIcon field should be a string key matching ICON_MAP above
//
// Usage:
//   import reelsData from '../data/reels.json';
//   <ReelCarousel reels={reelsData} />
//
//   — or fetch at runtime —
//   const [reels, setReels] = useState([]);
//   useEffect(() => { fetch('/reels.json').then(r => r.json()).then(setReels); }, []);
//   <ReelCarousel reels={reels} />
// ─────────────────────────────────────────────────────────────────────────

// export default function ReelCarousel({ reels = [] }) {
//   // Resolve tagIcon string → component once, keep everything else as-is
//   const REELS = reels.map(r => ({
//     ...r,
//     tagIcon: ICON_MAP[r.tagIcon] ?? null,
//   }));
...
//           {/* Card content — only rendered on active slide */}
//           {i === active && (
//             <div style={{
//               position: "absolute", inset: 0,
...
//         {REELS.map((r, i) => (
//           <Box
//             key={r.id} flex={1} h="2.5px" borderRadius="full"
//             bg="rgba(255,255,255,0.18)" overflow="hidden"
//             cursor="pointer" onClick={() => goTo(i)}
//           >
//             <Box
...
//       {/* ── Pause / play ── */}
//       <Box
//         position="absolute" top="40px" right={4} zIndex={10}
//         onClick={togglePause} cursor="pointer"
//         w="26px" h="26px" borderRadius="full"
//         bg="rgba(0,0,0,0.5)" backdropFilter="blur(8px)"
//         border="1px solid rgba(255,255,255,0.12)"
...
//       <Box position="absolute" left={0} top={0} w="38%" h="full"
//         zIndex={9} onClick={prev} cursor="pointer" />
//       <Box position="absolute" right={0} top={0} w="38%" h="full"
//         zIndex={9} onClick={next} cursor="pointer" />
//     </Box>
//   );
// }
// // TODO: IconMap should also be passed as an argument to ReelsCarosuel -> reels,iconMap
