...
/>
      ...
      {/* ── Tablet sidebar edge toggle tab ── */}
      {/* Sits just outside the sidebar right border, vertically centered.
          Always visible so users always know how to open/close the sidebar. */}
      <MotionBox
        display={{ base: "none", sm: "flex", lg: "none" }}
        position="fixed"
        top="50%"
      ...
      >
        ...
        >
          {/* Gradient accent line on the visible edge */}
          <Box
            position="absolute"
            right={0} top={0} bottom={0}
            w="2px"
            bgGradient="linear(to-b, #c026d3, #7c3aed)"
            opacity={0.7}
          />
      ...
      {/* SVG gradient defs — defined once, referenced by all icons */}
      ...
      >
        ...
        {LINKS.map(({ name, href }) => {
          ...
          return (
            ...
            >
              ...
              {/* Icon — gradient fill when active, muted when not */}
              {LinkIcon && (
                <Box
                  as="span"
                  sx={active ? {
                    "& svg": { stroke: "url(#nav-grad)" },
                  } : {
                    "& svg": { stroke: useColorModeValue("var(--chakra-colors-gray-400)", "var(--chakra-colors-whiteAlpha-500)") },
                  }}
                  style={{ transition: "all 0.18s ease" }}
                >
                  <LinkIcon size={22} strokeWidth={active ? 2.2 : 1.8} />
                </Box>
              )}
            </Box>
          );
        ...
        {/* Theme toggle — gradient when active isn't applicable, use static gradient */}
        <Box
          display="flex" flexDirection="column" alignItems="center"
          justifyContent="center"
          px={4} py={2} borderRadius="12px" minW="52px"
          ...
          sx={{
            "& svg": {
              stroke: useColorModeValue("var(--chakra-colors-gray-400)", "var(--chakra-colors-whiteAlpha-500)"),
              transition: "stroke 0.18s ease",
            },
            "&:hover svg": { stroke: "url(#nav-grad)" },
          }}
        >
          {colorMode === "light" ? <Moon size={22} strokeWidth={1.8} /> : <Sun size={22} strokeWidth={1.8} />}
        </Box>
